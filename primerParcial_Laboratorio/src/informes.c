...
int masPrestamos(eClientes clientes[], ePrestamo prestamos[], int tamC,
		int tamP, int estado) {
	...
	if (clientes != NULL && prestamos != NULL && tamC > 0 && tamP > 0) {
		for (i = 0; i < tamC; i++) {
			if (clientes[i].isEmpty == OCUPADO) {
				for (j = 0; j < tamP; j++) {
					if (prestamos[j].isEmpty == OCUPADO
							&& prestamos[j].clienteId == clientes[j].id) {
...
int clienteMasPrestamos(eClientes clientes[], ePrestamo prestamos[], int tamC,
		int tamP) {
	eClientes auxCliente;
	...
	if (prestamos != NULL && tam > 0) {
		...
		for (i = 0; i < tam; i++) {
			if (prestamos[i].importe == importe && prestamos[i].estado == ACTIVO) {
				printf("\n\t%d\t\t\t%d\t\t\t%d\t\t\t%d", prestamos[i].id,
						prestamos[i].clienteId, prestamos[i].importe,
						prestamos[i].cuotas);
				retorno = 1;
			}
		}
