...
void hardcodeo_clientes(eClientes *lista) {
	...
	char nombre[6][51] = { "Eduardo", "Maria", "Cristina", "Sabrina", "Gabriel",
			"Tomas" };
	char apellido[6][51] = { "Casarero", "Gomez", "Sanchez", "Villar", "Chaves",
			"Gallegos" };
	char cuil[6][50] = { "27111111111", "27222222221", "27333333331",
			"27444444441", "27555555551", "27666666661" };
...
int imprimirClientePrestamos(eClientes clientes[], ePrestamo prestamos[],
		int tamC, int tamP) {
	int retorno = -1;
	int i;
	int verifIndice;
			...
			for (i = 0; i < tamC; i++) {
				if (clientes[i].isEmpty == OCUPADO) {
					printf("\n\t%d\t\t%10s\t\t%10s\t\t%14s\t\t%d\t\n",
							...
							clientes[i].apellido, clientes[i].cuil,
							cantidadPrestamosCliente(clientes[i].id, prestamos,
									TAM_PRESTAMOS));
					retorno = 1;
